...
func (s *ratingRepoSuite) Test_SaveRating_SavesRating() {
	...
	err := s.repo.Create(context.Background(), model_test_helper.TestUserId, rating)
	require.NoError(s.T(), err)

	ratings, err := s.repo.GetAll(context.Background(), model_test_helper.TestUserId)
	require.NoError(s.T(), err)
...
func (s *ratingRepoSuite) Test_SaveRating_ReturnsError() {
	...
	err := s.repo.Create(ctx, "me", model_test_helper.ARatingForArtist("Bloodbath"))
	require.ErrorContains(s.T(), err, "context canceled")

	ratings, err := s.repo.GetAll(context.Background(), "me")
	require.NoError(s.T(), err)
...
func (s *ratingRepoSuite) Test_GetRatings_ReturnsAllRatingsForUser() {
	...
	err := s.repo.Create(context.Background(), model_test_helper.TestUserId, rating1)
	require.NoError(s.T(), err)
	err = s.repo.Create(context.Background(), model_test_helper.TestUserId, rating2)
	require.NoError(s.T(), err)
...
func (s *ratingRepoSuite) Test_GetRatings_ReturnsError() {
	...
	ratings, err := s.repo.GetAll(ctx, model_test_helper.TestUserId)
	require.ErrorContains(s.T(), err, "context canceled")
	require.Equal(s.T(), []model.Rating(nil), ratings)
}
